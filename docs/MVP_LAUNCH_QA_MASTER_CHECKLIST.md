...
- **EXT-013** — Aplicar `supabase/migrations-pending/20260811074500_process_refund_increment_atomic.sql` (via ferramenta de migration, com esse nome). Ordem obrigatória: **migration primeiro**, deploy do `webhook-asaas` depois. Sem ela o reembolso é fail-closed (500/retry) e nada é processado pela metade.
