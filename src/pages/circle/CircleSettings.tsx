      ...
      <Tabs defaultValue="profile">
        ...
        <TabsContent value="profile" className="space-y-6">
          <Card className="p-6 space-y-5">
            {/* Sync toggle — only for MEMBER role */}
            {!isOwnerOrAdmin && (
              <div className="flex items-center justify-between p-3 rounded-lg bg-muted/50">
                <div>
                  <p className="text-sm font-medium text-foreground">Usar perfil da Kivo</p>
                  <p className="text-xs text-muted-foreground">Sincronizar nome e avatar com sua conta Kivo</p>
                </div>
                <Switch
                  id="sync-kivo"
                  checked={form.sync_with_kivo}
                  onCheckedChange={(v) => setForm((p) => p && ({ ...p, sync_with_kivo: v }))}
                />
              </div>
            )}
            {isOwnerOrAdmin && (
              <div className="flex items-center gap-2 p-3 rounded-lg bg-primary/5 border border-primary/10">
                ...
                <p className="text-xs text-muted-foreground">
                  Como {member.role === "OWNER" ? "dono" : "admin"} da comunidade, seu perfil é sempre sincronizado com sua conta Kivo.
                </p>
              </div>
            )}
              ...
              <div className="relative">
                ...
                {(!form.sync_with_kivo || isOwnerOrAdmin) && (
                  <button
                    ...
                    disabled={uploadingAvatar || form.sync_with_kivo}
            ...
            <div className="space-y-1.5">
              ...
              <Input
                id="username"
                value={form.username}
                onChange={(e) => setForm((p) => p && ({ ...p, username: e.target.value.toLowerCase().replace(/[^a-z0-9_]/g, '') }))}
                placeholder="seu_username"
                className="h-10"
                maxLength={30}
              />
              <p className="text-xs text-muted-foreground">Usado para @menções. Apenas letras, números e _</p>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              />
              <p className="text-xs text-muted-foreground">
                {form.sync_with_kivo ? "Sincronizado com seu perfil Kivo" : "Este é o nome que outros membros verão."}
              </p>
            </div>
