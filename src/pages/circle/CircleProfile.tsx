  ...
  return (
    <div className="flex gap-6 p-4 md:p-6 max-w-5xl mx-auto w-full">
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {ownedCommunities.map((c: any) => (
                <CommunityCard key={c.id} community={c} onView={() => navigate(`/circles/${c.slug}/feed`)} />
              ))}
            </div>
          </div>
        )}
        ...
        <Card className="p-5 space-y-5 sticky top-20">
          ...
          {isOwnProfile && (
            <Button
              ...
              onClick={() => navigate(`/circles/${slug}/settings?section=profile`)}
            >
      ...
      <div className="lg:hidden fixed bottom-16 left-0 right-0 z-10 bg-background border-t p-3 flex items-center gap-3">
        ...
        {isOwnProfile && (
          <Button variant="outline" size="sm" className="text-xs h-7 shrink-0" onClick={() => navigate(`/circles/${slug}/settings?section=profile`)}>
            Edit
          </Button>
        )}
      </div>
