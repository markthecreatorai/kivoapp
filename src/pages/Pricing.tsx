...
const PLANS_BASE = [
  {
    ...
    icon: Zap,
    features: ["3 produtos", "100 leads", "Storefront básica", "Checkout integrado", "Com marca Kivo"],
  ...
  },
  {
    ...
    icon: Crown,
    popular: true,
    features: ["Produtos ilimitados", "5.000 leads", "Email marketing", "Programa de afiliados", "Domínio customizado", "Sem marca Kivo", "Analytics avançado"],
    excluded: [],
  },
  {
    id: "business",
    ...
    icon: Sparkles,
    features: ["Tudo do Creator", "White-label completo", "NFS-e automática", "Múltiplas circles", "API pública", "Suporte prioritário"],
    excluded: [],
  },
];
...
export default function Pricing() {
  ...
  const handleChoosePlan = (plan: typeof PLANS_BASE[number]) => {
    if (plan.code === "free") {
      navigate("/dashboard");
      return;
    }
    startPlanCheckout({ planCode: plan.code, sourceUI: "pricing_page" });
  };
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-5xl mx-auto px-4 py-12 md:py-20">
        ...
        <div className="text-center mb-12">
          <h1 className="text-3xl md:text-4xl font-bold text-foreground mb-3">Planos & Preços</h1>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {plans.map((plan) => {
            ...
            return (
              <Card
                key={plan.id}
                className={`relative transition-all hover:shadow-lg ${plan.popular ? "ring-2 ring-primary shadow-lg scale-[1.02]" : ""}`}
              >
                {plan.popular && (
                ...
                <CardContent className="space-y-4 pt-4">
                  ...
                  <Button
                    ...
                    disabled={loading && plan.code !== "free"}
                  >
                    {loading && plan.code !== "free" ? (
        ...
        <p className="text-center text-xs text-muted-foreground mt-8">
          Todos os planos incluem checkout integrado, PIX e cartão de crédito.
        </p>
      </div>
    </div>
  );
